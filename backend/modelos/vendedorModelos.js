...
export const inserta = async (vendedor) => {
  const { ci, nombre, paterno, materno, telefono, direccion } = vendedor;
  const [resultado] = await db.query('INSERT INTO VENDEDOR(ci, nombre, paterno, materno, telefono, direccion) values (?,?,?,?,?,?)', [ci, nombre, paterno, materno, telefono, direccion]);
  return resultado;
}
...
export const actualiza = async (id_vendedor, vendedor) => {
  const { ci, nombre, paterno, materno, telefono, direccion } = vendedor;
  await db.query('UPDATE vendedor SET ci = ?, nombre = ?, paterno = ?, materno = ?, telefono = ?, direccion = ? WHERE id_vendedor = ?', [ci, nombre, paterno, materno, telefono, direccion, id_vendedor]);
