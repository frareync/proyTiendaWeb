...
export const inserta=async (proveedor) => {
    const {nombre, paterno, materno, telefono, direccion, correo}=proveedor;
    const [resultado]=await db.query('INSERT INTO PROVEEDOR(nombre, paterno, materno, telefono, direccion, correo) values (?,?,?,?,?,?)',[nombre, paterno, materno, telefono, direccion, correo]);
    return resultado;
}
...
export const actualiza = async (id_proveedor, proveedor) => {
  const {nombre, paterno, materno, telefono, direccion, correo} = proveedor;
  await db.query('UPDATE proveedor SET nombre = ?, paterno = ?, materno = ?, telefono = ?, direccion = ?, correo = ? WHERE id_proveedor= ?', [nombre, paterno, materno, telefono, direccion, correo, id_proveedor]);
