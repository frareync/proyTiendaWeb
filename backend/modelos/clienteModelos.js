...
export const inserta=async (cliente) => {
    const {nombre, paterno, materno, nacionalidad}=cliente;
    const [resultado]=await db.query('INSERT INTO CLIENTE(nombre, paterno, materno, nacionalidad) values (?,?,?,?)',[nombre, paterno, materno, nacionalidad]);
    return resultado;
}
...
export const actualiza = async (id_cliente, cliente) => {
  const {nombre, paterno, materno, nacionalidad} = cliente;
  await db.query('UPDATE cliente SET nombre = ?, paterno = ?, materno = ?, nacionalidad = ? WHERE id_cliente = ?', [nombre, paterno, materno, nacionalidad, id_cliente]);
